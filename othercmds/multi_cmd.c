...
/*   Updated: 2022/08/25 18:28:09 by sgmira           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

void	processing_firstcmd(t_vars *vars, t_exenv exenv, int *fd)
{
...
void	processing_mdlcmd(t_vars *vars, t_exenv exenv, int *fd)
{
...
void	processing_lastcmd(t_vars *vars, t_exenv exenv, int *fd)
{
...
int    execute_multicmd(t_vars *vars, t_exenv exenv, int *fd)
{
	...
	if (pid1 == 0)
	{
		if (vars->i == 1)
        {
			processing_firstcmd(vars, exenv, fd);
            printf("First %s\n",exenv.args->arg[0]);
            // exit(1);
        }
		else if (vars->i == (vars->num))
        {
			processing_lastcmd(vars, exenv, fd);
            printf("Last  %s\n", exenv.args->arg[0]);
            // exit(1);
        }
		else
        {
			processing_mdlcmd(vars, exenv, fd);
            printf("Middle %s\n", exenv.args->arg[0]);
            // exit(1);
        }
	}
